...
bool  decendingly_by_response_ratio(tuple<string, double, int> a, tuple<string, double, int> b){
    return get<1>(a) > get<1>(b);
}
...
void highestResponseRatioNext(){
    vector<tuple<string, double, int>> present_processes; //<process_name, process_response_ratio, time_in_service>
    int j=0;
    for(int current_instant = 0; current_instant < last_instant; current_instant++){
        ...
        sort(all(present_processes), decendingly_by_response_ratio);
...
void aging(int originalQuantum){
    vector<tuple<int, int, int>>v; //prioritylevel, processIndex, total waiting time
    int j=0;
